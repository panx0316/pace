      ...
      <li>
		...
		<?php foreach ($hitos as $data_hitos){ ?>
          <li>
			...
			<?php foreach ($actividades as $data_actividades){ 
				if($data_hitos->P_ID_HITO == $data_actividades->P_ID_HITO){ ?>
              <li>	<a href="#"><?php echo $data_actividades->P_NOMBRE_ACTIVIDAD." - Fecha Inicio:". $data_actividades->P_FECHA_INICIO." / Fecha Término:". $data_actividades->P_FECHA_TERMINO." / Valor Asignado:". $data_actividades->P_VALOR ." / Avance:". $data_actividades->P_PORC_AVANCE."%" ?></a>
              </li>
			<?php }	} ?>
            </ul>
          </li>
