...
class Pace_model extends CI_Model{
	...
	return $query->result();
		...
		public function getProyectos2($proyecto = FALSE)
		{
		if($proyecto!=FALSE){
		$sql="select * from p_proyecto where P_ID_PROYECTO='{$proyecto}'";
	...
	public function getProyectos($proyecto = FALSE)
	{
	if($proyecto!=FALSE){
	$sql="select * from p_proyecto where P_ID_PROYECTO='{$proyecto}'";
	}
	...
	public function getResultados($id=FALSE)
	{
	if($id!=FALSE){
	$sql="select * from p_resultado where P_ID_ACTIVIDAD='{$id}' ";
	}
	else{
	$sql="select * from p_resultado";
	}
